...
Every package build regenerates the repository-root `thumb.png` from the current `main` branch of [se-mod-thumbnails](https://github.com/marnason/se-mod-thumbnails), resolved by Go at build time, using the text `SEpedia`, background color `#0000`, and padding `100`. The 1280×720 output remains tracked for repository previews, while packages always use the freshly generated image without comparing it to the committed copy.
