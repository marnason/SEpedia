...
2. **Catalog and advanced filters** — Focused physical-item/block/celestial categories; G-menu-aware block browsing; runtime source, grid-size, block-type, and flag facets; spawned-planet tracking; safe local-HUD suppression. Completed and accepted in game.
...
The catalog/filter/celestial iteration is complete and its advanced-filter performance has been accepted in game. Components remain the default browse category; recipes and generic definitions remain indexed for links without appearing as browse categories. Work can now proceed to broader definition detail while missing-dependency, dedicated-server, and release-hardening coverage remain later milestones.
