...
- For operator-attended MDK configuration in this workspace, use the existing WSL-native `mdkhub` launcher when it is available. Do not substitute or download a Windows build without explicit approval. Confirm that WSLg exposes the `MDK² Hub` window before reporting that the Hub opened; a surviving process alone is insufficient.
- Portable code and guidance MUST NOT contain machine-specific paths, users, Steam roots, or deployment destinations.
- GitHub CLI commands running without a checked-out repository MUST set `GH_REPO` explicitly. Recheck the [GitHub CLI environment documentation](https://cli.github.com/manual/gh_help_environment) when changing checkout-free jobs.
- GitHub immutable releases are enabled. Deleting an immutable release allows its Git ref to be deleted, but that tag name cannot be reused. Nightly automation MUST publish with a unique tag and may prune older nightlies only after the replacement succeeds; do not delete and recreate a fixed tag. Create immutable releases as drafts, attach and verify every asset, then publish through bounded, idempotent retries so transient provider failures are safe. Recheck the repository setting and [GitHub's immutable release documentation](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases) when changing this flow.
...
- Stable Workshop packages MUST contain a release-specific version marker so a source-identical release cannot become a silent no-op. Do not treat a successful SteamCMD exit alone as proof of publication; use bounded polling to confirm the public Workshop item's update time advances before publishing the GitHub release. Recheck Valve's [Workshop upload guidance](https://partner.steamgames.com/doc/features/workshop/implementation#SteamCmd) and [`GetPublishedFileDetails` Web API](https://partner.steamgames.com/doc/webapi/ISteamRemoteStorage#GetPublishedFileDetails) when changing this verification.
- SteamCMD Workshop VDF changenotes MUST use literal line-feed characters for line breaks; escaped `\n` text is rendered literally. Validate the generated manifest contains the intended literal line break before upload.
...
- Rich HUD terminal integrations MUST explicitly enable the mod root as well as their pages. Runtime verification MUST confirm the SEpedia root and expected controls appear in the terminal; a successful client-registration or initialization log alone is insufficient.
...
- Catalog results and resolved detail-page relationship targets MUST share the common visibility evaluator. Relationship composers MUST NOT reimplement Enabled, Public, Survival, or Source checks; runtime verification MUST exercise filtered targets through both forward and reverse relationships.
...
- Width-dependent Rich HUD collections MUST materialize their child nodes during the content rebuild using the current or last-known width, then use `Layout` only to reflow them when dimensions change. Runtime verification MUST cover selecting the already displayed entry again, not only initial and different-entry navigation.
...
- Custom Rich HUD input handlers MUST gate mouse and keyboard processing on [`HudMain.InputMode`](https://zachhembree.github.io/RichHudFramework.Client/api/RichHudFramework.UI.Client.HudMain.html#RichHudFramework_UI_Client_HudMain_InputMode) and require a neutral bind state after `NoInput` or focus transitions. Cached hover state alone is not input authority because transient bind states can span those transitions.
