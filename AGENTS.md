...
- Definition `Icons[]` values are GUI texture paths, not Rich HUD transparent-material identifiers. Do not conclude that icons are infeasible from a direct `Material(path)` test alone, but only package bridge experiments exposed through the script whitelist, Rich HUD's API, or load-time content definitions.
- Space Engineers creates renderer transparent materials from loaded definitions before session scripts start. Dynamic Rich HUD definition icons therefore require load-time path aliases or a matching material already registered by the definition's own content context; the public definition list is read-only for this purpose at runtime.
