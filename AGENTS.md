...
- The current icon pipeline uses load-time path aliases or transparent materials registered by the definition's own content context because renderer materials are created before session scripts start. Treat this as the boundary of the implemented pipeline, not proof that no alternative engine or framework resolution route exists.
